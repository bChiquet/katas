...
public class BloomFilter {
    ...
    //Magic number explained here : http://pages.cs.wisc.edu/~cao/papers/summary-cache/node8.html
    double MAGIC_NUMBER = 0.6185;
    ...
    BloomFilter(int expectedNumberOfElements, double acceptableFalsePositiveRate)
        ...
        if (multiplier == 0) {
            throw new FilterCantBuildException();
        }
    ...
    private BitSet hash(String word) {
        ...
        for (int i = 0; i < numberOfHashPerWord; i++) {
            hash.set(rand.nextInt(filterSize), true);
        }
        return hash;
    }
}
